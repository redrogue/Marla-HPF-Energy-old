<%@ Page Language="VB" MasterPageFile="~/website.master" AutoEventWireup="false"
    CodeFile="africa.aspx.vb" Inherits="_Default" Title=" Africa Projects - HPF Energy Services - Total Support for all your Pipeline Product requirements from the highest of Quality Sources." %>
                    ...
                    <asp:UpdatePanel ID="upProjects" runat="server">
                        <ContentTemplate>
                            ...
                            <asp:Repeater ID="rptProjects" runat="server" DataSourceID="accDS">
                                <ItemTemplate>
                                    <h3 style="margin-top: 20px;">
                                        <asp:Literal ID="litTitle" runat="server" Text='<%# trim(Eval("Title")) %>' /><a style="float: right;"
                            ...
                            <asp:AccessDataSource ID="accDSRegion" DataFile="~/App_Data/hpf_updateable.mdb" runat="server"
                                SelectCommand="SELECT DISTINCT Location FROM contracts WHERE [World Region] = 'Africa' ORDER BY Location ASC">
                            </asp:AccessDataSource>
                            <asp:AccessDataSource ID="accDS" DataFile="~/App_Data/hpf_updateable.mdb" runat="server"
                                SelectCommand="SELECT [ID], [Title], [User/Operator], [Contractor], [Location], [World Region], [Products Supplied], [Contract Type], [Complete] FROM contracts WHERE [World Region] = 'Africa' AND [Location] LIKE '%' + @Location + '%'">
                                <SelectParameters>
